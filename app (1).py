...
st.markdown("""
    ...
    /* This prevents the user from closing the sidebar so it never gets stuck */
    ...
    /* --- DASHBOARD: PRIYA'S PREMIUM CARDS --- */
...
with st.sidebar:
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        # Watermark '0.00' added via placeholder
...
if df.empty:
    ...
    with c_left:
        ...
        if not exp_only.empty:
            fig = px.area(exp_only.groupby(['date', 'category'])['amount'].sum().reset_index(), x='date', y='amount', color='category', template="plotly_dark")
